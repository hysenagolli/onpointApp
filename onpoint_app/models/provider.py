...
class Provider:
    ...
    def get_all_professions(cls):
        query = 'SELECT DISTINCT profession FROM providers;'
        results = connectToMySQL(cls.db_name).query_db(query)
        professions = [result['profession'] for result in results]
        return professions
    ...
    def create_provider(cls, data):
        query = "INSERT INTO providers (first_name, last_name, profession, email, password, about, verification_code) VALUES ( %(first_name)s, %(last_name)s, %(profession)s,%(email)s,%(password)s,%(about)s, %(verification_code)s);"
        return connectToMySQL(cls.db_name).query_db(query, data)
